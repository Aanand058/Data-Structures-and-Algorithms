...
/**
 * The DoublyLinkedPlaylistManager class manages a playlist of songs using a
 * doubly linked list structure.
 * This class allows for efficient addition, removal, and navigation of songs in
 * the playlist.
 * Each song is represented by a Node, which contains links to both the next and
 * previous songs,
 * enabling traversal in both directions. The class provides methods to add
 * songs to the playlist,
 * remove songs by index, display all songs, and navigate through the playlist.
 * It also maintains
 * the current song being played, allowing users to easily move to the next or
...
public class DoublyLinkedPlaylistManager {
    ...
    public void removeFavoriteSong(int index) {
        ...
        if (index == 1) {
            ...
            head = head.next;
            if (head != null) {
                // TODO 22: updates the previous reference of the head with null if the first
                // node is removed.\
                head.previous = null;
            }
            return;
        ...
        if (current == null) {
            ...
            if (current.previous != null) {
                // TODO 23: update the next reference of the previous node to skip over the
                // current node.
                current.previous.next = current.next;
            }
            if (current.next != null) {
                // TODO 24: update the previous reference of the next node to point to the
