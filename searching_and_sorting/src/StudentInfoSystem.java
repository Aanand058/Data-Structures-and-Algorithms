...
public class StudentInfoSystem {
    ...
    public List<Subject> sortSubjectsByName() {
        ...
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - 1 - i; j++) {
                Subject current = subjectsByName.get(j);
                Subject next = subjectsByName.get(j + 1);
    ...
    public List<Student> sortStudentsByName(){
        ...
        for (int i = 1; i < n; i++) {
            ...
            // Move elements of students[0..i-1], that are greater than key.name, to one position ahead
...
//        }
        ...
        while (left <= right) {
            ...
            int midNumber = midStudent.getStudentNumber();

            if (midNumber == studentNumber) {
                return midStudent;  // Found the student
            } else if (midNumber < studentNumber) {
